...
Create a new environment called `testing-feature`, which can be deployed on demand from any branch whose name starts with `testing-feature/` (e.g. `testing-feature/new-awesome-feature`), rather than from `main`.
...
- Deployment is triggered manually: pushes to a `testing-feature/*` branch run the regular CI checks (code quality, e2e tests, image build) automatically, but the actual deployment to `testing-feature` only runs when someone explicitly dispatches the `ci-pipeline.yml` workflow via `workflow_dispatch`, and only after those CI checks have passed. This avoids a formal reviewer/approval gate while still ensuring nothing broken gets deployed.
