...
import { IClient, Parameter, RequestBuilder } from "~/services/cms/strapi/client";
...
it("should return a menu", async () => {
  ...
  expect(mockObject.getDocument).toBeCalledWith("menus", new RequestBuilder()
    .setFilter({
      field: "slug",
      value: `1_de`,
    })
...
it("should return a page", async () => {
  ...
  expect(mockObject.getDocument).toBeCalledWith(pageName, new RequestBuilder()
    .setLocale(Locale.de)
    .toRequest());
...
it("should return a page by slug", async () => {
  ...
  expect(mockObject.getDocument).toBeCalledWith("pages", new RequestBuilder()
    .setLocale(Locale.de)
    .addFilter({
      field: "slug",
      value: slug,
