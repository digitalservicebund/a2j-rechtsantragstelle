...
const envMap: Array<
  [keyof ReturnType<typeof config>, string, string, string | undefined]
> = [
  ...
  ["SAML_IDP_CERT", "/etc/saml/idp_cert", "FAKE_SAML_IDP_CERT", undefined],
  [
...
describe("environment variables", () => {
  describe.each(envMap)("%s", (key, path, value, defaultValue) => {
    beforeEach(() => {
    ...
    it(`return ${defaultValue} as the default value`, () => {
      vi.stubEnv(key, undefined);
      expect(config()[key]).toBe(defaultValue);
    });
  });
});
