...
describe("Vault", () => {
  ...
  describe("pack", () => {
    it("should return a Buffer", () => {
      const result = Vault.pack(mockData, mockUuid);
      expect(Buffer.isBuffer(result)).toBe(true);
    });

    it("should produce non-JSON buffer with userKey", () => {
      const packed = Vault.pack(mockData, mockUuid, mockUserKey);
  ...
  describe("unpack", () => {
    it("should decrypt valid encrypted data", () => {
      const encrypted = Vault.pack(mockData, mockUuid, mockUserKey);
      expect(Vault.unpack(encrypted, mockUuid, mockUserKey)).toEqual(mockData);
    });

    it("should parse legacy unencrypted data automatically", () => {
      const legacyData = Buffer.from(JSON.stringify(mockData), "utf8");
      expect(Vault.unpack(legacyData, mockUuid)).toEqual(mockData);
    });

    it("should return null if the userKey is wrong (Auth Tag failure)", () => {
      const encrypted = Vault.pack(mockData, mockUuid, mockUserKey);
      expect(Vault.unpack(encrypted, mockUuid, "wrong-key")).toBeNull();
    });

    it("should return null if the UUID is different (Key Derivation failure)", () => {
      const encrypted = Vault.pack(mockData, "uuid-a", mockUserKey);
      expect(Vault.unpack(encrypted, "uuid-b", mockUserKey)).toBeNull();
    });
    ...
    it("should return null and log error if encrypted data is missing a key", () => {
      const consoleSpy = vi
        ...
        .mockImplementation(() => {});
      const encrypted = Vault.pack(mockData, mockUuid, mockUserKey);
      expect(Vault.unpack(encrypted, mockUuid)).toBeNull();
    ...
    it("should return null if the buffer is tampered with", () => {
      ...
      expect(Vault.unpack(encrypted, mockUuid, mockUserKey)).toBeNull();
    });

    it("should return null for an empty buffer", () => {
      expect(Vault.unpack(Buffer.alloc(0), mockUuid)).toBeNull();
    });

    it("should return null for non-JSON data", () => {
      const badData = Buffer.from("this is definitely not json", "utf8");
      expect(Vault.unpack(badData, mockUuid)).toBeNull();
    });

    it("should return null if the encrypted buffer is truncated", () => {
      const encrypted = Vault.pack(mockData, mockUuid, mockUserKey);
      const truncated = encrypted.subarray(0, 10);
      expect(Vault.unpack(truncated, mockUuid, mockUserKey)).toBeNull();
    });

    it("should handle an empty object correctly", () => {
      const empty = {};
      const packed = Vault.pack(empty, mockUuid, mockUserKey);
      expect(Vault.unpack(packed, mockUuid, mockUserKey)).toEqual(empty);
    });
  });
  ...
  describe("Rotation Simulation", () => {
    it("should support a sequential rotation flow", () => {
      ...
      const blob1 = Vault.pack(mockData, mockUuid, keyA);
      const current = Vault.unpack(blob1, mockUuid, keyA);
      expect(current).toEqual(mockData);

      const updatedData = { ...current, page: 2 };
      const blob2 = Vault.pack(updatedData, mockUuid, keyB);
      expect(Vault.unpack(blob2, mockUuid, keyA)).toBeNull(); // Verify Key A no longer works for blob2
      expect(Vault.unpack(blob2, mockUuid, keyB)).toEqual(updatedData); // Verify Key B works for blob2
    });

    it("should produce different ciphertexts for the same data (IV rotation)", () => {
      const blob1 = Vault.pack(mockData, mockUuid, mockUserKey);
      const blob2 = Vault.pack(mockData, mockUuid, mockUserKey);
