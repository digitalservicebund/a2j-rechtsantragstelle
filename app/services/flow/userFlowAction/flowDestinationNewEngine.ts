...
export const flowDestinationNewEngine = <C extends PageConfigMap>(
  ...
  const { flowId, arrayIndexes, stepId } =
  ...
  const prunedUserDataPruned = addPageDataToUserData(userDataPruned, {
    arrayIndexes,
  });
  const flowSessionEngineDestination = createFlowSession(
    compiledStaticFlow,
    prunedUserDataPruned as Parameters<typeof createFlowSession>[1],
    stepId,
  );

  const nextStepId =
    flowSessionEngineDestination.nextPath ??
    flowSessionEngineDestination.initialPath;
