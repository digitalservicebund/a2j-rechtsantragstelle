...
const getSessionHighlightTexts = async (cookieHeader: string | null) => {
  const { getSession } = getSessionManager("highlight-texts");
...
export const loader = async ({ params, request }: LoaderFunctionArgs) => {
  ...
  const [contentData, { headers, csrf }, , highlightTextSession] =
    await Promise.all([
      ...
      getSessionHighlightTexts(cookieHeader),
    ]);
  ...
  const highlightText = contentData.getHighlightText(
    arraySummaryData,
    highlightTextSession,
  );
  ...
  let autoGeneratedSections: SummaryItem[] = [];

  const isFinalSummaryPage = pathname.endsWith(
    "/antrag/abgabe/zusammenfassung",
  );

  if (isFinalSummaryPage && (await isFeatureFlagEnabled("showAutoSummary"))) {
    const stepStates = flowController.stepStates();
    autoGeneratedSections = await generateSummaryFromUserData(
      userData,
      flowId,
      stepStates,
      translations,
