...
import { KernMissingDataList } from "../kern/common/KernMissingDataList";
...
describe("MissingDataList", () => {
  it("should render only items in the Warning state", () => {
    const { getByText, queryByText } = render(
      <KernMissingDataList navItems={mockNavItems} shouldRender={true} />,
    );
  ...
  it("should not render without shouldRender", () => {
    const { container } = render(<KernMissingDataList navItems={[]} />);
    expect(container).toBeEmptyDOMElement();
  });
  it("should not render with shouldRender=False", () => {
    const { container } = render(
      <KernMissingDataList navItems={[]} shouldRender={false} />,
    );
    expect(container).toBeEmptyDOMElement();
  });
});
