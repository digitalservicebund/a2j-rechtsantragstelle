import { type Elternteil } from "~/domains/nachlass/erbschein/erbfolge/elternteilePages";
import { type Kind } from "~/domains/nachlass/erbschein/erbfolge/kinderPages";
...
import {
  ...
  WHOLE,
  type Fraction,
} from "~/domains/nachlass/erbschein/shared/fraction";
...
export function calculateInheritance(input: InheritanceInput): HeirShare[] {
  ...
  if (input.spouse) {
    const living1stOrderStaemme = has1stOrder
      ...
      : 0;
    const share = spouseShare("unknown", living1stOrderStaemme, has2ndOrder);
