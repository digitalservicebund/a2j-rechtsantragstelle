...
describe("createBankInformation", () => {
  it("should create document with bank information when account holder is provided", () => {
    ...
    createBankInformation(true, mockDoc, mockStruct, userDataMock);

    expect(mockDoc.struct).toHaveBeenCalledWith("P", {}, expect.any(Function));
    expect(mockDoc.text).toHaveBeenCalledWith(
      "Kontoinhaber: Test-Test Müller | IBAN: DE68 5001 2345 6789 0000 00",
      expect.anything(),
      expect.anything(),
    );
  });
  ...
  it("should default to vorname and nachname if account holder is missing", () => {
    ...
    createBankInformation(true, mockDoc, mockStruct, userDataNoAccountHolder);

    expect(mockDoc.struct).toHaveBeenCalledWith("P", {}, expect.any(Function));
    expect(mockDoc.text).toHaveBeenCalledWith(
      "Kontoinhaber: Max Mustermann | IBAN: DE68 5001 2345 6789 0000 00",
      expect.anything(),
      expect.anything(),
    );
  });
  ...
  it("should not add bank information if IBAN is missing", () => {
    ...
    createBankInformation(true, mockDoc, mockStruct, userDataWithoutIban);
  ...
  it("should handle special characters in kontoinhaber", () => {
    ...
    expect(mockDoc.text).toHaveBeenCalledWith(
      "Kontoinhaber: Jöhn Dœ!💰 | IBAN: DE68 5001 2345 6789 0000 00",
      expect.anything(),
      expect.anything(),
    );
  });
  ...
  it("should use Artifact markContent when isLastPage is false", () => {
    ...
    createBankInformation(false, mockDoc, mockStruct, userDataMock);
